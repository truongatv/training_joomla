...
class vmTheme extends vmTemplate  {
	...
	function vmBuildFullImageLink( $product ) {
		...
		if ( @$product["product_url"] ) {
			$product_image = "<a style=\"text-decoration:none\" href=\"". $product["product_url"]."\" title=\"".$product['product_name']."\" target=\"_blank\">";
		...
		/* Show the Thumbnail with a Link to the full IMAGE */
			...
			if( empty($product['product_full_image'] ) ) {
				$product_image = "<img style=\"text-decoration:none\" src=\"".VM_THEMEURL.'images/'.NO_IMAGE."\" alt=\"".$product['product_name']."\" border=\"0\" />";
			}
			else {
				...
				if( @$_REQUEST['output'] != "pdf" && $this->get_cfg('useLightBoxImages', 1 ) ) {
					...
					$product_image = str_replace('<a href', '<a style="text-decoration: none" href', $product_image);
				}
				elseif( @$_REQUEST['output'] != "pdf" ) {
					...
					$product_image = "<a style=\"text-decoration:none\" href=\"$imageurl\" target=\"_blank\">"
